...
- **Nivel 3** — código de un caso _análogo_, no del suyo.
...
- Mensajes de commit en español, en imperativo, explicando el _por qué_ cuando no es obvio.
...
npm run format     # Prettier (format:check para CI)
npm run db:start   # supabase start — Postgres/Auth/Storage locales (requiere Docker)
npm run db:stop    # supabase stop
npm run db:reset   # recrea la base y aplica supabase/migrations/
npm run db:types   # genera src/lib/database.types.ts desde el schema local
# test           → se configura en la Fase 8
```
